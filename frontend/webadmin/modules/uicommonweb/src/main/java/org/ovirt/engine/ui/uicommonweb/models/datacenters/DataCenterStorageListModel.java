			...
			_asyncQuery.asyncCallback = new INewAsyncCallback() { public void OnSuccess(Object model, Object result)
				{
					...
					java.util.ArrayList<storage_domains> items = dcStorageModel.getItems() != null ? new java.util.ArrayList<storage_domains>(Linq.<storage_domains>Cast(dcStorageModel.getItems())) : new java.util.ArrayList<storage_domains>();
			...
			_asyncQuery.asyncCallback = new INewAsyncCallback() { public void OnSuccess(Object model, Object result)
				{
					...
					for (storage_domains a : list)
					{
						...
						if (!Linq.IsSDItemExistInList(items, a.getid()) && a.getstorage_domain_type() == dcStorageModel.getStorageDomainType())
						{
							if (dcStorageModel.getStorageDomainType() == StorageDomainType.Data && a.getstorage_type() == dcStorageModel.getEntity().getstorage_pool_type() && a.getstorage_domain_shared_status() == StorageDomainSharedStatus.Unattached)
							{
								if (dcStorageModel.getEntity().getStoragePoolFormatType() == null)
								{
									...
									if (a.getStorageStaticData().getStorageFormat() != StorageFormatType.V1 && dcStorageModel.getEntity().getcompatibility_version().compareTo(version3_0) < 0)
									{
										continue;
									}
									addToList = true;
								}
								else if ((StorageFormatType)dcStorageModel.getEntity().getStoragePoolFormatType() == a.getStorageStaticData().getStorageFormat())
								{
									addToList = true;
								}
							}
							else if (dcStorageModel.getStorageDomainType() == StorageDomainType.ImportExport && a.getstorage_domain_shared_status() == StorageDomainSharedStatus.Unattached)
							{
								addToList = true;
							}
